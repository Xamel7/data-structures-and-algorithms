...
const checkValues = (obj, value) => {
  // Solution code here...
  // Convert the object values to an array using Object.values()
...
const updateNumbers = (obj) => {
  // Solution code here...
  // Get the entries of the object using Object.entries()
  const entries = Object.entries(obj);

  // Map the entries to the desired format
  const updatedData = entries.map(([name, phoneNumber]) => `${name}: ${phoneNumber}`);
...
const getHouses = (arr) => {
  let houses = [];
  // Solution code here...
  return arr.reduce((houses, character) => {
    if (character.house) {
...
const hasChildrenValues = (arr, character) => {
  ...
  // Find the character object in the array
  const characterObj = arr.find((char) => char.name === character);
  // If the character object is found, check if it has the 'children' property with non-empty array values
  if (characterObj) {
    const children = characterObj.children;
