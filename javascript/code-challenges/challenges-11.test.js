...
const count = (target, input) => {
  ...
  let count = 0;
  input.forEach(arr => {
    count += arr.reduce((acc, num) => acc + (num === target ? 1 : 0), 0);
  });
  return count;
};
...
let findShortest = (data) => {
  // Solution code here...
  const shortestCharacter = data.sort((a, b) =>
    parseInt(a.height) - parseInt(b.height))[0];
  return shortestCharacter ? shortestCharacter.name : '';
};
